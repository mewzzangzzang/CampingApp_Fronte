...
class CommViewHolder(val binding: ItemDonmBinding) : RecyclerView.ViewHolder(binding.root)

class CommListAdapter(val context: CampCommActivity, val datas: List<GlampList>?) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder =
        CommViewHolder(ItemDonmBinding.inflate(LayoutInflater.from(parent.context), parent, false))

    // 리사이클러뷰
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        val binding = (holder as CommViewHolder).binding
